...
# TODO make difficulty and lvl container
...
class GameRect(object):
    ...
    def __contains__(self, otherobj):
        if (self.x <= otherobj.x) and ((self.x + self._width) >= (otherobj.x + otherobj._width)) and (
                self.y <= otherobj.y) and ((self.y + self._height) >= (otherobj.y + otherobj._height)):
            return True
        else:
            return False
...
class Text(object):
    def __init__(self, surface, posx, posy, legend=None, fontsize=32, fontcolor=Colors.BLACK, bgcolor=Colors.BLACK,
...
class Score(Text):
    def __init__(self, surface, posx, posy, score=0, fontsize=32, fontcolor=Colors.BLACK, bgcolor=Colors.WHITE, label="Score: {0}"):
        Text.__init__(self, surface, posx, posy, label.format(score), fontcolor=fontcolor, fontsize=fontsize,
                      bgcolor=bgcolor)
        self.score = score

    def modifyscore(self, newscore, label="Score: {0}"):
        self.score = newscore
        self.legend = label.format(self.score)

    def addscore(self, deltascore, label="Score: {0}"):
        self.score += deltascore
        self.legend = label.format(self.score)

    def resetscore(self, label="Score: {0}"):
        self.score = 0
        self.legend = label.format(self.score)
...
class Button(GameRect):
    def __init__(self, surface, display, posx, posy, width, height, buttonaction=None, forecolor=Colors.GRAY,
                 bgcolor=Colors.GRAY, pressedcolor=Colors.GRAY, legend=None, fontcolor=Colors.BLACK,
                 font='freesansbold.ttf', fontsize=32, sound=None):
